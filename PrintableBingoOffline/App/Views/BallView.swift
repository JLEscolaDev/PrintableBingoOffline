...
struct BallView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            let maxDimension = max(geometry.size.height, geometry.size.width)
            ZStack {
                Circle().fill(color)
                Circle()
                    .fill(Color.clear)
                ...
                Circle()
                    .fill(Color.white)
                    .frame(width: maxDimension*0.7, height: maxDimension*0.7)
                ...
                Text("\(number)")
                    ...
                    .frame(width: maxDimension*0.63, height: maxDimension*0.63)
                    .clipShape (Circle())
            }
            .overlay(
